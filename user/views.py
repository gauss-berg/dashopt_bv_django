...
def send_active_email(email_address, verify_url):
    ...
    res = send_mail(subject, '', '1375779612@qq.com', [email_address], html_message=html_message)
    return res


def find_pass_view(request):
    json_str = request.body
    json_obj = json.loads(json_str)
    number = random.randint(100000, 999999)
    subject = '达达t商城找回密码邮件'
    html_message = '尊敬的用户您好，您的验证码为%d' % number

    send_mail(subject, '', '1375779612@qq.com', [json_obj['email']], html_message=html_message)
...
def update_view(request):
    ...
    json_obj = json.loads(json_str)
    m = hashlib.md5()
    m.update(json_obj['password'].encode())
    password_m = m.hexdigest()
    try:
        user = UserProfile.objects.get(email=json_obj['email'])
...
def modify_view(request):
    ...
    json_obj = json.loads(json_str)
    m = hashlib.md5()
    m.update(json_obj['new_password'].encode())
    password_m = m.hexdigest()
    m1 = hashlib.md5()
    m1.update(json_obj['old_password'].encode())
    password_m1=m1.hexdigest()
    try:
        user = UserProfile.objects.get(username=json_obj['username'], password=password_m1)
        user.password = password_m
        user.save()
    except:
        return JsonResponse({'code': 403, 'msg': 'error'})
